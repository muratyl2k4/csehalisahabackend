...
def secure_file_upload(instance, filename):
    """Dosya adını güvenli hale getirir (uuid + uzantı)"""
...
class Player(models.Model):
    """Oyuncu Profili (User ile ilişkili)"""
    ...
    def calculate_overall(self):
        """Mevkiye göre ağırlıklı ortalama hesaplar"""
        p = self.position
    ...
    def initialize_stats(self):
        """Pozisyona göre başlangıç statlarını ayarla (~75 Overall)"""
        p = self.position
        if p == 'KL':
